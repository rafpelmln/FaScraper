...
var chromeTab = {
	...
	tableData = {},
	o = 1e3;
console.log("URL Tab Saat Ini : ", chromeTab.url);
var a = null;
async function r() {
	if (typeof chromeTab.url === "string" && chromeTab.url.toLowerCase().match(/\/\/[a-z]+\.liiinkedin\.com/)) {
		$("#waitHeader").hide();
		l("Kami tidak dapat mengumpulkan data dari LinkedIn. Maaf atas ketidaknyamanannya. Jika ada pertanyaan lain, silakan hubungi kami di info@webrobots.io.", "noResponseErr", false, true);
	} else {
		q();
		D();
		setTimeout(function() {
...
function S(retry = false) {
    if (retry) {
        chrome.tabs.reload(chromeTab.id, {}, function() {
            setTimeout(D, 1000);
        });
    } else {
        l("Failed to load data. Please try again.", "noResponseErr", false, true);
    }
}
...
function l(e, o, a, r) {
	if ("" === e) return $("#" + o).hide();
	$("#" + o).show().text(e), a && I(), r && (window._gaq = window._gaq || []).push(["_trackEvent", "Error", tableData.startingUrl || chromeTab.url, e, 1])
}
...
function u(e) {
	...
	}), console.log("Kolom serupa: " + d), console.log("Kolom dengan baris yang sama: " + f), console.log("Kolom dengan beberapa baris: " + p), console.log("Nama kolom yang dibuat:", r);
	var g = {
		fields: r,
	...
	return console.log("CSV Dibuat:", g), g
}
...
function v() {
	console.log("Menampilkan Pratinjau");
...
function b() {
	...
	$("#waitHeader").hide(), l("Instant Data belum mendukung pengambilan data dari situs ini. Tim kami telah diberitahu dan akan mencoba menambah dukungan di masa depan. Terima kasih telah mencoba!", "noResponseErr", false, true)
}
...
function x(e, o) {
	if (!e) return chromeTab.reloaded ? S(true) : (chromeTab.reloaded = true, chrome.tabs.reload(chromeTab.id, {}, function() {
		setTimeout(D, 1e3)
	}));
	...
	$("#wrongTable").show(), tableData.config.infinateScrollChecked && $("#infinateScroll").click(), chrome.tabs.sendMessage(chromeTab.id, {
		action: "getTableData"
	}, function(e) {
		e.tableId == tableData.tableId && (tableData.pages || ($("#nextButton").show(), tableData.nextSelector = k(), console.log("Sonraki Seçici » " + tableData.hostName, tableData.nextSelector), tableData.nextSelector && chrome.tabs.sendMessage(chromeTab.id, {
			action: "markNextButton",
			selector: tableData.nextSelector
		}, function(e) {
			e.error || ($("#nextButton").hide(), $("#startScraping").show())
		})), $("#wait").hide(), $("#content").show(), $('#content').addClass('d-flex'), l('Untuk menelusuri beberapa halaman, unduh data atau temukan tombol "Berikutnya"!', "instructions"), tableData.data = e.data, tableData.pages = 1, tableData.lastRows = e.data.length, tableData.tableSelector = e.tableSelector, tableData.workingTime = 0, L(), v(), $(".download-button").show(), $("#csv").off("click").click(function() {
...
function y(e) {
	if (typeof e !== "string" || !e.trim()) return "unknown";
	try {
		var t = new URL(e).hostname.split(".");
		return t[0].indexOf("www") > -1 ? t[1] : t[0];
	} catch (err) {
		console.error("Invalid URL:", e, err);
		return "unknown";
	}
}
...
function D() {
    chrome.tabs.sendMessage(chromeTab.id, {
        action: "findTables",
        robot: a
    }, function(e) {
        if (chrome.runtime.lastError) {
            console.error("Chrome API Error:", chrome.runtime.lastError);
            l("Error communicating with tab. Please refresh the page and try again.", "error", true, true);
            return;
        }
        x(e, true);
    });
...
function q() {
	$("#stopScraping").click(I), $("#crawlDelay").bind("propertychange change click keyup input paste", function() {
		var e = $(this).val();
	if (isNaN(e) || e < 0 || parseInt(1e3 * e) >= tableData.config.maxWait) return l("Nilai waktu tunggu minimum tidak valid", "inputError");
		...
		var e = $(this).val();
	if (isNaN(e) || parseInt(1e3 * e) <= tableData.config.crawlDelay) return l("Nilai waktu tunggu maksimum tidak valid", "inputError");
...
function I() {
	tableData.scraping = false, console.log("Ekstraksi Dihentikan!"), $("#startScraping").show(), $("#stopScraping").hide(), l("Ekstraksi dihentikan. Silakan unduh data atau lanjutkan penjelajahan.", "instructions")
}
...
function L() {
	$("#stats").empty().append($("<div>", {
		text: "Halaman yang Diekstrak : " + tableData.pages
	})).append($("<div>", {
		text: "Total Baris yang Diekstrak : " + tableData.data.length
	})).append($("<div>", {
		text: "Diekstrak dari Halaman Terakhir : " + tableData.lastRows
	})).append($("<div>", {
		text: "Waktu yang Berlalu : " + parseInt(tableData.workingTime / 1e3) + " detik"
...
r(), $("#wrongTable").click(function() {
	...
	! function r() {
		...
		N() && (s = i), e(s, function() {
			...
			chrome.tabs.sendMessage(chromeTab.id, {}, function(t) {
				e(void 0 !== t)
			})
		})
	}()
