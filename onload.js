...
async function p(e, t) {
	if (t) {
		...
		}), void console.log("Tabel tidak ditemukan!");
		o.length || (o = [{}]);
		var a = r(i);
		return v(a.text()) ? (e({
			error: "Tabel belum berubah. Jika halaman terakhir belum tercapai, coba tingkatkan penundaan navigasi.",
			errorId: "finished"
	...
	}), console.log("Data Tabel yang Dikumpulkan:", c), e({
		data: c,
		tableId: l,
	...
	h = function(t) {
		return t.preventDefault(),
			function(t) {
				...
				var n = w(t.target);
				$(t.target).addClass("tablescraper-next-button"), console.log("Pemilih Tombol Berikutnya:", n), e({
					selector: n
				})
			}(t), !1
...
function S(e, t, n) {
	...
	return r ? (r.last().addClass("tablescraper-next-button"), n ? t({}) : ($("*").off("click", h).off("mouseenter", d), void setTimeout(function() {
		...
		error: "Tombol Berikutnya Tidak Ditemukan!",
		errorId: "error"
	} : {
		error: "Tidak ada lagi tombol 'Berikutnya'; Pemindaian selesai. Unduh berkas CSV atau Excel.",
		errorId: "finished"
	})
}
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  console.log("Pesan diterima:", request);

  if (request.action === "findTables" || request.action === "nextTable") {
    if (request.action === "findTables") {
      a(request.robot);
    } else {
      l = (l + 1) % o.length;
    }
    ...
    p(sendResponse, request.selector);
    return true; // ← PENTING buat async response
  } else if (request.action === "getNextButton") {
    x(sendResponse);
    return true;
  } else if (request.action === "clickNext") {
    S(request.selector, sendResponse);
    return true;
  } else if (request.action === "scrollDown") {
    c();
    N(0, sendResponse);
    return true;
  } else if (request.action === "markNextButton") {
    S(request.selector, sendResponse, true);
    return true;
  } else {
    sendResponse({});
  }
